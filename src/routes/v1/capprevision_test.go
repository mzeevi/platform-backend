...
const (
	cappRevisionNamespace = "test-namespace-capp-revisions"
		...
		"ShouldSucceedGettingCappRevisions": {
			...
			want: want{
				...
				response: types.CappRevisionList{Count: 2, CappRevisions: []cappv1alpha1.CappRevision{
					types.GetBareCappRevision(cappRevisionName+"-1", cappRevisionNamespace, map[string]string{labelKey + "-1": labelValue + "-1"}, map[string]string{}),
					types.GetBareCappRevision(cappRevisionName+"-2", cappRevisionNamespace, map[string]string{labelKey + "-2": labelValue + "-2"}, map[string]string{}),
				}},
			},
		},
...
func TestGetCappRevision(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"ShouldSucceedGettingCappRevision": {
			...
			want: want{
				statusCode: http.StatusOK,
				response: types.GetBareCappRevisionType(cappRevisionName+"-1", cappRevisionNamespace,
