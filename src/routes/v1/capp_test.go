...
func createTestCapp(name, namespace string, labels, annotations map[string]string) {
	capp := mocks.GetCapp(name, namespace, labels, annotations)
...
func createTestCappWithHostname(name, namespace string, labels, annotations map[string]string) {
	capp := mocks.GetCappWithHostname(name, namespace, labels, annotations)
	err := dynClient.Create(context.TODO(), &capp)
	if err != nil {
		panic(err)
	}
	...
	cases := map[string]struct {
		...
		"ShouldSucceedGettingCapps": {
			...
			want: want{
				...
				response: types.CappList{Count: 4, Capps: []types.CappSummary{
		...
		"ShouldSucceedGettingCappsWithLabelSelector": {
			...
			want: want{
				statusCode: http.StatusOK,
				response: types.CappList{Count: 1, Capps: []types.CappSummary{
					{Name: cappName + "-1", URL: fmt.Sprintf("https://%s-%s.%s", cappName+"-1", cappNamespace, mocks.Domain), Images: []string{mocks.CappImage}},
				},
				},
			},
...
func TestGetCapp(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"ShouldSucceedGettingCapp": {
			...
			want: want{
				statusCode: http.StatusOK,
				response:   mocks.GetCappType(cappName+"-1", cappNamespace, map[string]string{labelKey + "-1": labelValue + "-1"}, nil),
			},
		},
		"ShouldFailWithBadRequestInvalidURI": {
			requestParams: requestParams{
...
func TestCreateCapp(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"ShouldSucceedCreatingCapp": {
			bodyParams: bodyParams{
				capp: mocks.GetCappTypeWithoutStatus(cappName+"-6", cappNamespace, map[string]string{labelKey + "-6": labelValue + "-6"}, nil),
			},
			want: want{
				statusCode: http.StatusOK,
				response:   mocks.GetCappTypeWithoutStatus(cappName+"-6", cappNamespace, map[string]string{labelKey + "-6": labelValue + "-6"}, nil),
			},
		},
		"ShouldFailWithBadRequestInvalidURI": {
			bodyParams: bodyParams{
				capp: mocks.GetCappTypeWithoutStatus(cappName+"-6", "", map[string]string{labelKey + "-6": labelValue + "-6"}, nil),
			},
			want: want{
				statusCode: http.StatusBadRequest,
				response:   types.Capp{},
		...
		"ShouldFailWithBadRequestBody": {
			bodyParams: bodyParams{
				capp: mocks.GetCappTypeWithoutStatus("", cappNamespace, map[string]string{labelKey + "-6": labelValue + "-6"}, nil),
			},
			want: want{
				statusCode: http.StatusBadRequest,
				response:   types.Capp{},
...
func TestUpdateCapp(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"ShouldSucceedUpdatingCapp": {
			bodyParams: bodyParams{
				capp: mocks.GetCappType(cappName+"-7", cappNamespace, map[string]string{labelKey + "-7-updated": labelValue + "-7-updated"}, nil),
			},
			want: want{
				statusCode: http.StatusOK,
				response:   mocks.GetCappType(cappName+"-7", cappNamespace, map[string]string{labelKey + "-7-updated": labelValue + "-7-updated"}, nil),
			},
		},
		"ShouldFailWithBadRequestInvalidURI": {
			bodyParams: bodyParams{
				capp: mocks.GetCappType(cappName+"-7", "", map[string]string{labelKey + "-7": labelValue + "-7"}, nil),
			},
			want: want{
				statusCode: http.StatusBadRequest,
				response:   types.Capp{},
...
func TestDeleteCapp(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"ShouldSucceedDeletingCapp": {
			...
			want: want{
				statusCode: http.StatusOK,
				response:   types.CappError{Message: fmt.Sprintf("Deleted capp %q in namespace %q successfully", cappName+"-9", cappNamespace)},
			},
		},
		"ShouldFailWithBadRequestInvalidURI": {
			requestParams: requestParams{
